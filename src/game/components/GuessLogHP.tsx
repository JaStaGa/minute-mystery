            ...
            <table className="w-full text-sm border-collapse">
                <thead>
                    <tr className="[&>th]:bg-zinc-800 [&>th]:text-white [&>th]:px-3 [&>th]:py-2">
                        <th>Name</th><th>House</th><th>Gender</th><th>Year</th>
                        <th>Hair</th><th>Ancestry</th><th>Status</th>
                    </tr>
                </thead>
                <tbody>
                    {attempts.map((name, i) => {
                        ...
                        const aliveText = aliveGuess === null ? "—" : aliveGuess ? "Alive" : "Deceased";

                        const cell = (match: boolean, v: string) =>
                            <td className={`px-3 py-2 text-center ${match ? "bg-lime-400 text-black" : "bg-zinc-800 text-white"}`}>{v || "—"}</td>;

                        return (
                            <tr key={`${name}-${i}`} className="[&>td]:border [&>td]:border-zinc-700">
