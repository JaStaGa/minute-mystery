...
import { getPOKCharacters } from "@/game/themes/pokemon/adapter";
import GuessLogPOK from "@/game/components/GuessLogPOK";
import styles from "./pok-theme.module.css";
...
export default function PokemonGame() {
    const [all, setAll] = useState<POKFields[]>([]);
    ...
    useEffect(() => {
        setAll(getPOKCharacters());
        setReady(true);
    }, []);

    useEffect(() => {
        (async () => {
            const id = await getGameId(supabase, "pokemon");
            setGameId(id);
    ...
    function onGuess(fd: FormData) {
        ...
        const target = state.target as unknown as POKFields | null;
    ...
    useEffect(() => {
        if (state.status === "playing" && state.mistakes >= 5) {
            setEndReason("lost");
            const t = state.target as unknown as POKFields | null;
    ...
    useEffect(() => {
        ...
        (async () => {
            await upsertHighScore(supabase, "pokemon", state.score);
            if (gameId) {
                    ...
                    <div className="max-w-3xl mx-auto h-full game-screen">
                        {state.status === "idle" && (
                            <div className="h-full grid place-content-center content-center gap-6 text-center">
                                <div className={styles.panel}>
                                    <h1 className={styles.pokTitle}>POKÉMON GUESSING GAME</h1>
                                    <div className="mt-3 flex flex-col items-center gap-3">
                                        <button className={styles.pokButton} onClick={start}>Start</button>
                                        <Link href="/g/pokemon/leaderboard" className={styles.pokButton}>Leaderboard</Link>
                                    </div>
                                </div>
                            </div>
                        )}
                        ...
                        {state.status === "playing" && state.target && (
                            <div className="space-y-4">
                                ...
                                <form action={onGuess} className={styles.formColumn} autoComplete="off">
                                    ...
                                    <datalist id="pok-names">
                                        {all.map((c) => (<option key={c.name} value={c.name} />))}
                                    </datalist>

                                    <button type="submit" className={styles.pokButton}>Guess</button>
                                </form>

                                <div className={styles.tableWrap}>
                                    <GuessLogPOK
                                        round={{
                                            targetId: (state.target as unknown as POKFields).name,
                        ...
                        {ended && (
                            <div role="dialog" aria-labelledby="gameOverTitle" className={`space-y-4 text-center ${styles.endBlock}`}>
                                <div className={styles.panel}>
                                    <h2 id="gameOverTitle" className={styles.pokTitle} style={{ margin: 0, fontSize: "clamp(1.6rem, 5.5vw, 2.4rem)" }}>
                                    ...
                                    <div className="mt-3 flex justify-center gap-3">
                                        <button className={styles.pokButton} onClick={start}>Play again</button>
                                        <Link href="/g/pokemon/leaderboard" className={styles.pokButton}>View leaderboard</Link>
                                    </div>
                                </div>
                            </div>
                        )}
